...
// Custom order of Health Centers
const customOrder = [
  "Shaviyani Atoll Hospital",
  "Kanditheemu Health Centre",
  "Noomaraa Health Centre",
  "Goidhoo Health Centre",
  "Feydhoo Health Centre",
  "Feevaku Health Centre",
  "Bilehfahi Health Centre",
  "Foakaidhoo Health Centre",
  "Narudhoo Health Centre",
  "Maroshi Health Centre",
  "Lhaimagu Health Centre",
  "Komandoo Health Centre",
  "Maaungoodhoo Health Centre",
  "Milandhoo Health Centre"
];
...
async function loadData() {
  ...
  try {
    ...
    if (!Array.isArray(data) || data.length === 0) {
      container.innerHTML = '<p class="no-results text-center text-gray-500">No data found.</p>';
      return;
    }
    ...
    // Sort centers by custom order
    const groupedCenters = Object.keys(grouped).sort((a, b) => {
...
<thead class="bg-[#00bcd4] text-white">
    ...
    container.innerHTML = `<p class="no-results text-center text-red-500">Failed to load data: ${error.message}</p>`;
    console.error(error);
  }
}
...
function filterData() {
  ...
  if (!anyVisible && !document.querySelector('.no-results-message')) {
    ...
    msg.className = 'no-results no-results-message text-center text-gray-500';
    ...
    headers.forEach(th => {
      th.addEventListener('click', () => {
        ...
        rowsArray.sort((a, b) => {
          ...
          if (colIndex === 0) { // Phone numeric sort
